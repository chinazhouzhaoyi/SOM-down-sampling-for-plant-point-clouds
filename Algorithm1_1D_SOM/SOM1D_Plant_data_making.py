...
file:  Algorithm1_1D_SOM/SOM1D_Plant_data_making.py
                        ...
                        if self.label_sample:# The sampling results include xyz coordinates and labels
                                    ...
                                    #1D SOM sampling
                                        ...
                                        for q in range(self.npoints):
                                            point_sample[q, :3]=SOM_sample[q][0, :3]
                                        for j in range(self.npoints):
                                            Dstore = []
                                            sampler = SOM_sample[j][0, :3]
...
def parse_args():
    ...
    parser.add_argument('--num_point', type=int, default=512, help='Point sampler Number, recommended [128, 256, 512, 1024]')
    parser.add_argument('--sigma', type=float, default=1.5, help='Control parameters for activating domain radius, recommended range[1.2, 1.5]')
    parser.add_argument('--use_som_sample', type=bool, default=True, help='use som sampling instead of FPS sampling') 
    parser.add_argument('--label_sample', type=bool, default=True, help='sample with instance label instead of merely points') 
    ...
    parser.add_argument('--expand_data', type=bool, default=False, help='expand the sapmling number by repeating the samplig')
