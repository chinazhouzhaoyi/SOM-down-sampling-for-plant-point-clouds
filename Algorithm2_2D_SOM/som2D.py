...
file:  Algorithm2_2D_SOM/som2D.py
about: main entrance for using 2D SOM for point cloud downsampling, targeting a single 2D topology shape 
author: Zhaoyi Zhou
date: 2023-5-16
'''
...
def trainSom(data, x, y, epochs, sigma, verbose=False):
    ...
    """
    som = utils.MiniSom(x, y, 3, sigma=sigma, learning_rate=0.5, random_seed=1)
...
def getSkeleton(organs, nodesNum, sigma):
    """
    for input 2D structure, returns the nodes of the skeleton
    ...
    for i, o in enumerate(organs): 
        oshape = o.shape
        weight = trainSom(o, x=int(np.sqrt(nodesNum)), y=int(np.sqrt(nodesNum)), epochs=int(5 * oshape[0]), sigma=sigma)
