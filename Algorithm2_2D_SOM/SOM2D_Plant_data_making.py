...
file:  Algorithm2_2D_SOM/SOM2D_Plant_data_making.py
...
def SOM_point_sample(data, resampleNum, sigma):
    ...
    organs = []
    organs.append(data)
    # print(np.sqrt(resampleNum))
    if np.sqrt(resampleNum) % 1 != 0:
        Num = int(np.sqrt(resampleNum)) + 1
        assignNum = [Num * Num]
...
def parse_args():
    ...
    parser.add_argument('--num_point', type=int, default=81, help='Point sampler Number, recommended [64, 81, 100]')
